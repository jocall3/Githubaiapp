...
import { ExpansionJob } from '../types';
import { Spinner } from './Spinner';

const StatusIcon: React.FC<{ status: ExpansionJob['status'] }> = ({ status }) => {
    switch (status) {
        case 'queued': 
...
interface ExpansionProgressProps {
  jobs: ExpansionJob[];
  onClose: () => void;
  isComplete: boolean;
}

export const ExpansionProgress: React.FC<ExpansionProgressProps> = ({ jobs, onClose, isComplete }) => {
  ...
  const activeJobs = jobs.filter(j => j.status === 'generating' || j.status === 'committing');
    ...
    <div className="fixed inset-0 bg-gray-950 bg-opacity-80 flex items-center justify-center z-50">
      <div className="bg-gray-850 p-6 rounded-lg shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col border border-gray-700">
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
            <h2 className="text-2xl font-bold text-purple-400">AI Expansion Progress</h2>
            {isComplete && (
              <button
                onClick={onClose}
        ...
        <div className="mb-4 flex-shrink-0">
            <div className="flex justify-between text-sm text-gray-300 mb-1">
                ...
                <span>{successCount} successful commits</span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 flex-grow min-h-0">
            <div className="col-span-1 bg-gray-900 rounded-md p-4 overflow-y-auto">
                 <h3 className="text-lg font-semibold mb-2 text-gray-200">Expansion Blueprint</h3>
                 <ul className="space-y-1">
                    {jobs.map(job => (
                        <li key={job.id} className="flex items-center justify-between text-sm p-1.5 bg-gray-800 rounded">
                           <div className="flex items-center gap-3 overflow-hidden">
                               <StatusIcon status={job.status} />
                               <span className="truncate" title={job.newFilePath}>{job.newFilePath}</span>
                           </div>
            ...
            <div className="col-span-2 bg-gray-900 rounded-md p-4 flex flex-col">
                <h3 className="text-lg font-semibold mb-2 text-gray-200">Live Code Generation ({activeJobs.length} active)</h3>
                {activeJobs.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow min-h-0">
                       {activeJobs.slice(0, 4).map(job => (
                           <div key={job.id} className="flex-grow flex flex-col min-h-0 bg-gray-850 rounded-lg p-2">
                               <p className="text-blue-300 font-mono text-xs mb-2 truncate" title={job.newFilePath}>
                                 {job.status === 'generating' ? 'Generating' : 'Committing'}: <span className="font-bold">{job.newFilePath.split('/').pop()}</span>
                               </p>
                               <div className="bg-gray-950 rounded p-2 flex-grow overflow-y-auto">
                                    <pre className="text-xs text-gray-300 whitespace-pre-wrap break-words">
                                        <code>{job.generatedContent}</code>
                                    </pre>
                               </div>
                           </div>
                       ))}
